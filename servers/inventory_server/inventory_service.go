...
func (s *inventoryServer) CheckInventory(ctx context.Context, req *inventory_v1_pb.InventoryCheckRequest) (*inventory_v1_pb.InventoryCheckResponse, error) {
	...
	var crust_types []order_management_v1_pb.Pizza_CrustType
	var crust_sizes []order_management_v1_pb.Pizza_Size
	...
	toppings_count := make(map[order_management_v1_pb.Pizza_Topping]int)
	for _, topping := range toppings {
		toppings_count[topping]++
	}
	crust_types_count := make(map[order_management_v1_pb.Pizza_CrustType]int)
	for _, crust_type := range crust_types {
		crust_types_count[crust_type]++
	}
	crust_sizes_count := make(map[order_management_v1_pb.Pizza_Size]int)
